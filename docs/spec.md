...
- **People are named, not id'd**: Slack delivers every mention as a bare id (`<@U0EXAMPLE456>`), in the instruction, the quoted thread context and the image labels alike. One pass over the finished turn text resolves each id through cached `users.info` and renders it `@Display Name`, then appends a single `[Slack ids — @Name = <@ID>; …]` line so a reply can still deliberately notify someone. Display names are sanitized to one short unmarked-up line — they are user-authored text sitting next to the instruction. An unresolvable id, a failed lookup or a token without `users:read` leaves the mention exactly as Slack delivered it; the coordinator names people and never posts a bare id.
