...
- **Reference verbatims** (fixed by the mock): autonomy gate `🚦 <command> on <worktree> — go?`; worker gate = verbatim question + numbered options + "Reply in this thread"; queue `⏳ Queued (5 active sessions)…`; close 🔚 summary; cost `💸 This thread has cost $5.03…`; third party "v1: only <@U0EXAMPLE456> can drive me." (the mention list names every allowed user, issue #93); reboot `⚠️ Restarted — <repo>#<n> was in flight: <state>. Reply to resume supervision.`
