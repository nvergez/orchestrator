...
- Every root **and every reply** must @mention the bot — plain thread replies are not delivered
  to the app (workspace event-subscription problem, #38). Remove the mentions from these scripts
  once #38 is fixed.
...
| S5 | Mid-flight status (mock E) | Reply `@bot where is <task> at?` while a delegation runs | Snapshot answer from task-list/worktree ps, **zero 🚦** (#45); card stays the living surface |
