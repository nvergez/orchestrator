...
- **Blocking** — GitHub native issue-dependencies are **not** used (not reliably scriptable). Blocking is a **body convention**: a line `**Bloqué par :** #N, #M` in the ticket body. A ticket is **unblocked** when every issue it lists is **closed**.
...
- **Frontier** — open sub-issues of the map that are **unassigned** and whose every "Bloqué par" issue is **closed**.
...
- **New tickets** — create the issue, add its `wayfinder:<type>` label, link it as a sub-issue of the map
  ...
  and add any `**Bloqué par :**` line. Assets (research md, prototypes) are **linked** from the issue, not pasted into it.
