...
export class BootReconciler {
  ...
  private async reconcileThread(threadTs: string, channelId: string, observed: Observations | undefined): Promise<void> {
    ...
    await this.surface.settleReconciled(threadTs, channelId, closed.map((item) => item.kind));
    ...
    const fingerprint = items
      ...
      .sort()
      .join('|');
    if (closed.length === 0 && this.store.getReconcileFingerprint(threadTs, channelId) === fingerprint) {
    ...
    try {
      await this.surface.post(threadTs, notice, channelId);
    } catch (error) {
    ...
    this.store.setReconcileFingerprint(threadTs, fingerprint, channelId);
    this.logger.info(
   ...
   */
  private async peekWorkerDones(threadTs: string, channelId: string): Promise<OrchestrationMessage[]> {
