...
export class GateRelay implements SessionRelay {
   ...
   */
  decorateReply(threadTs: string, text: string, channelId = ''): string {
   ...
   */
  sanctionsSend(threadTs: string, command: string, channelId = ''): boolean {
    ...
    return (
      this.store.listPendingGates(threadTs, channelId).some((gate) => gate.workerHandle === handle) ||
      ...
      (hasFlag(tokens, '--enter') &&
        this.store.listPendingStalls(threadTs, channelId).some((stall) => stall.workerHandle === handle))
    );
  }

  /** Registry enforcement on `orchestration reply` — non-reply commands pass. */
  prepare(threadTs: string, command: string, channelId = ''): PrepareVerdict {
    const segments = commandSegments(command);
    ...
    if (gate === undefined || gate.threadTs !== threadTs || (channelId !== '' && gate.channelId !== channelId)) {
    ...
    if (gate.status === 'answered') {
      ...
      this.lastReply.set(slackThreadKey(threadTs, channelId), { msgId: gate.msgId, outcome: 'correction' });
   ...
   */
  private prepareSend(threadTs: string, channelId: string, command: string, segments: string[][]): PrepareVerdict {
   ...
   */
  private sendGate(threadTs: string, channelId: string, handle: string): PendingGateRow | undefined {
    const last = this.lastReply.get(slackThreadKey(threadTs, channelId));
    if (last !== undefined) {
    ...
    if (this.store.listPendingStalls(threadTs, channelId).some((stall) => stall.workerHandle === handle)) {
      return undefined;
    }
  ...
  async observe(threadTs: string, command: string, stdout: string, channelId = ''): Promise<void> {
    try {
  ...
  private async observeReply(threadTs: string, channelId: string, tokens: string[], stdout: string): Promise<void> {
    const msgId = flagValue(tokens, '--id');
    ...
    if (parseOrcaEnvelope(stdout) === null) {
      ...
      if (this.store.getGate(msgId)?.status === 'pending') {
        this.lastReply.set(slackThreadKey(threadTs, channelId), { msgId, outcome: 'failed' });
      }
      return;
    }
    const gate = this.store.getGate(msgId);
    if (gate === undefined || !this.store.answerGate(msgId)) return;
    this.lastReply.delete(slackThreadKey(threadTs, channelId));
    this.logger.info(
      { threadTs: gate.threadTs, msgId, workerHandle: gate.workerHandle },
      'gate answered — human reply relayed down',
    );
    await this.surface.settleRoot(gate.threadTs, gate.channelId);
  }
   ...
   */
  private async observeSend(threadTs: string, channelId: string, tokens: string[], stdout: string): Promise<void> {
    ...
    if (hasFlag(tokens, '--enter')) {
      ...
      if (nudged) await this.surface.settleRoot(threadTs, channelId);
    }
    const key = slackThreadKey(threadTs, channelId);
    const last = this.lastReply.get(key);
    if (last !== undefined && this.store.getGate(last.msgId)?.workerHandle === handle) {
      this.lastReply.delete(key);
  ...
  private async answerBySend(threadTs: string, channelId: string, msgId: string, handle: string): Promise<void> {
    if (!this.store.answerGate(msgId)) return;
    this.logger.info(
      { threadTs, msgId, workerHandle: handle },
      'gate answered via the terminal send fallback',
    );
    await this.surface.settleRoot(threadTs, channelId);
  }
}
