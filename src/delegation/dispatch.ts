...
export interface DelegationCoordinatorOptions {
  ...
  /** Legacy single-channel test/default; production passes channel per call. */
  channelId?: string;
...
export class DelegationCoordinator implements DispatchPreparer, DispatchObserver {
  ...
  constructor(options: DelegationCoordinatorOptions) {
    ...
    this.defaultChannelId = options.channelId ?? '';
  ...
  async prepare(threadTs: string, command: string, signal?: AbortSignal, channelId = this.defaultChannelId): Promise<PrepareVerdict> {
    ...
    if (creates.length === 1) return this.prepareCreate(threadTs, channelId, command, creates[0] as string[], signal);
   ...
   */
  private async prepareDispatch(threadTs: string, channelId: string, tokens: string[]): Promise<PrepareVerdict> {
  ...
  async observe(threadTs: string, command: string, stdout: string, channelId = this.defaultChannelId): Promise<void> {
    try {
  ...
  private async observeCreate(threadTs: string, channelId: string, tokens: string[], stdout: string): Promise<void> {
    const tracker = this.tracker(threadTs, channelId);
    ...
    try {
      pending.cardTs = await this.surface.post(threadTs, this.renderCard(pending), channelId);
    } catch (error) {
    ...
    await this.surface.ackWorking(threadTs, channelId);
  }

  /** A successful tui-idle wait clears that handle for injection (spec §5). */
  private observeTerminalWait(threadTs: string, channelId: string, tokens: string[], stdout: string): void {
    ...
    if (pending === undefined) {
      ...
      try {
        if (pending.cardTs === null) {
          ...
          await this.surface.update(pending.cardTs, this.renderCard(pending), channelId);
        }
      } catch (error) {
   ...
   */
  abandonThread(threadTs: string, channelId = this.defaultChannelId): void {
    const tracker = this.threads.get(slackThreadKey(threadTs, channelId));
   ...
   */
  hasUndispatched(threadTs: string, channelId = this.defaultChannelId): boolean {
    return (this.threads.get(slackThreadKey(threadTs, channelId))?.pending.size ?? 0) > 0;
  }
