...
Classify repo requests as Question (an answer, no changes) or Change (a pull request). A Question is an actual interrogation — "how does this work", "why does it break", "where does that live" — where the person wants to know something. Anything that asks for the code to end up different is a Change, however softly it is phrased ("it should really…", "this is broken", "we need a retry there"). In doubt, Change: the pull request IS the proposal — nobody merges it but a human, so opening one costs the thread less than a wall of text asking whether to open one. "Do it" after an answer, or "no, fix it", means a Change on that same subject and repo. Answer repo-less turns (available repos, delegation status) yourself without a worker.
