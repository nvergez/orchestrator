...
export function buildRuntime(options: RuntimeOptions): Runtime {
  ...
  const sessions = new SessionManager({
    ...
    onTurnStart: (threadTs, channelId) => surface.ackWorking(threadTs, channelId),
    onTurnEnd: (threadTs, channelId) => surface.settleTurnEnd(
      threadTs,
      channelId,
      delegations.hasUndispatched(threadTs, channelId),
    ),
    logger,
  });
